 ...
 * @version    $Id: Pdf.php 1569 2011-01-19 00:58:00Z bento.vilas.boas@gmail.com $
...
class Bvb_Grid_Deploy_Pdf extends Bvb_Grid implements Bvb_Grid_Deploy_DeployInterface {
    ...
    public function calculateCellSize()
    {
        ...
        foreach ($titles as $titulos) {
            if ((@$titulos['field'] != $this->getInfo('hRow,field') && $this->getInfo('hRow,title') != '') 
    ...
    public function deploy()
    {
        ...
        if (@$this->_deploy['noPagination'] != 1) {
            $page->drawText(
                $this->__($this->_deploy['page']) . ' ' . $pagina . '/' . $totalPaginas, 
                $page->getWidth() - (strlen($this->__($this->_deploy['page'])) * $cellFontSize) - 50, 
                40, 
                $this->getCharEncoding()
           );
        }
        ...
        foreach ($titulos as $value) {
            if (($value['field'] != $this->getInfo('hRow,field') && $this->getInfo('hRow,title') != '') 
                || $this->getInfo('hRow,title') == ''
            ) {
                if ((int) $la == 0) {
                    $largura1 = 40;
