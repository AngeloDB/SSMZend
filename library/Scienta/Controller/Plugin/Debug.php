 ...
 * @copyright  Copyright (c) 2008-2009 Joakim Nygård (http://jokke.dk)
 */
class Scienta_Controller_Plugin_Debug extends Zend_Controller_Plugin_Abstract
{
    protected $db           = array();
    ...
    public function mark($name) {
        if (isset($this->timer['user'][$name]))
            $this->timer['user'][$name] = round((microtime(true)-$_SERVER['REQUEST_TIME'])*1000, 2)-$this->timer['user'][$name];
        else
            $this->timer['user'][$name] = round((microtime(true)-$_SERVER['REQUEST_TIME'])*1000, 2);
    }
    
    public function preDispatch(Zend_Controller_Request_Abstract $request) 
    {
        $this->timer['preDispatch'] = round((microtime(true)-$_SERVER['REQUEST_TIME'])*1000, 2);
    }
    
    public function postDispatch(Zend_Controller_Request_Abstract $request) 
    { 
        $this->timer['postDispatch'] = round((microtime(true)-$_SERVER['REQUEST_TIME'])*1000, 2);
    }
    ...
    protected function getVersionPanel()
    {
        return '<div id="scienta_version" class="scienta_panel">'.
               '<h4>Scienta ZF Debug Bar v'.$this->_version.'</h4>'.
    ...
    protected function getFilesPanel()
    {
        ...
        if (isset($this->timer['user']) && count($this->timer['user'])) {
            foreach ($this->timer['user'] as $name => $time) {
                $html .= ''.$name.': '.$time.' ms<br>';
            }
        }
        $html .= 'Controller: '.($this->timer['postDispatch']-$this->timer['preDispatch']).' ms<br>';
