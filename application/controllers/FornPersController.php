...
class FornPersController extends Zend_Controller_Action
    {
    ...
    public function listAction()
        ...
        $defaultNamespace->cant2 = $translate->_("Fornitori personale");
        $currGrid = new Custom_XmlGrid();
        $grid = $currGrid->getGrid($this, 'forn_pers');
    ...
    public function newAction()
        {
        $defaultNamespace = new Zend_Session_Namespace('Default');
        $defaultNamespace->title1 = ' - Nuovo fornitore';
        $currForm = new Custom_XmlForm();
        $currForm->initForm($this->db, 'forn_pers');
        $currForm->setCurrAction($defaultNamespace->baseUrl . '/forn-pers/new');
    ...
    public function editAction()
        ...
        $currForm = new Custom_XmlForm();
        $currForm->initForm($this->db, 'forn_pers', $currId);
        $currForm->setCurrAction($defaultNamespace->baseUrl . '/forn-pers/edit');
        $form = $currForm->getForm();
        if ($this->_request->isPost())
            {
            $formData = $this->_request->getPost();
...
//        $defaultNamespace->title1 = $title;
    ...
    public function deleteAction()
        ...
        $request = $this->getRequest();
        $currId = $request->getParam('id');
        $sql = "DELETE FROM forn_pers WHERE idFornPers='" . $currId . "'";
        $qry = $this->db->query($sql);
        $this->_redirect('/forn_pers/list');
        // action body
        }
