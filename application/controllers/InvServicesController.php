...
class InvServicesController extends Zend_Controller_Action
{
...
// action body
    ...
    public function listAction()
        ...
        $defaultNamespace->cant2 = $translate->_("Servizi da fatturare");
        $currGrid = new Custom_XmlGrid();
        $grid = $currGrid->getGrid($this, 'inv_services');
    ...
    public function newAction()
        {
        $defaultNamespace = new Zend_Session_Namespace('Default');
        $defaultNamespace->title1 = ' - Nuovo servizio da fatturare';
        $currForm = new Custom_XmlForm();
        $currForm->initForm($this->db, 'inv_service');
        $currForm->setCurrAction($defaultNamespace->baseUrl . '/inv-services/new');
        ...
        if ($this->_request->isPost())
            ...
            if ($currId = $currForm->insertData($formData))
                {
                $this->_redirect('/inv-services/list');
                }
            else
                {
    ...
    public function editAction()
        ...
        $currForm = new Custom_XmlForm();
        $currForm->initForm($this->db, 'inv_service', $currId);
        $currForm->setCurrAction($defaultNamespace->baseUrl . '/inv-services/edit');
        $form = $currForm->getForm();
        if ($this->_request->isPost())
            {
            $formData = $this->_request->getPost();
    ...
    public function deleteAction()
        ...
        $request = $this->getRequest();
        $currId = $request->getParam('id');
        $sql = "DELETE FROM tab_contr_md WHERE IdTabContrMd='" . $currId . "'";
