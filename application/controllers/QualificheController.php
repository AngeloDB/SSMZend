...
class QualificheController extends Zend_Controller_Action
    {
    ...
    public function listAction()
        ...
        $defaultNamespace->cant2 = $translate->_("Qualifiche");
        $currGrid = new Custom_XmlGrid();
        $grid = $currGrid->getGrid($this, 'qualifiche');
    ...
    public function newAction()
        {
        $defaultNamespace = new Zend_Session_Namespace('Default');
        $defaultNamespace->title1 = ' - Nuova qualifica';
        $currForm = new Custom_XmlForm();
        $currForm->initForm($this->db, 'qualifica');
        $currForm->setCurrAction($defaultNamespace->baseUrl . '/qualifiche/new');
        ...
        if ($this->_request->isPost())
            ...
            if ($currId = $currForm->insertData($formData))
                {
                $this->_redirect('/qualifiche/list');
                }
            else
                {
    ...
    public function editAction()
        ...
        $currForm = new Custom_XmlForm();
        $currForm->initForm($this->db, 'qualifica', $currId);
        $currForm->setCurrAction($defaultNamespace->baseUrl . '/qualifiche/edit');
        $form = $currForm->getForm();
        if ($this->_request->isPost())
            {
            $formData = $this->_request->getPost();
...
//        $defaultNamespace->title1 = $title;
    ...
    public function deleteAction()
        ...
        $request = $this->getRequest();
        $currId = $request->getParam('id');
        $sql = "DELETE FROM qualifiche WHERE idQualifica='" . $currId . "'";
